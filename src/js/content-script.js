...
var isDisabled = !options.enableSortByMyUpvotes &&
  ...
  function getAnswers(options){
    ...
    all.forEach(function(answer, position) {
        ...
        // Up vote
        if (options.enableSortByMyUpvotes && answer.find('.vote-up-on').length) {
          upVoted.push(answer);
        }

        // Down vote
         else if (options.enableSortByMyUpvotes && answer.find('.vote-down-on').length) {
  ...
  function markAnswersAsUpVoted(answers){
    answers.forEach(function(answer) {
      answer = jQuery(answer);
      answer.find('.vote-up-off').addClass('vote-up-on');
    });
  }
